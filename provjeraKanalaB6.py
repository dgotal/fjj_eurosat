...
model_path = 'eurosat_efficientnetb6_model.h5'
...
def load_and_preprocess_image(path):
    with rasterio.open(path) as src:
        ...
        # image = np.delete(image, 12, axis=-1)
        # image = np.delete(image, 8, axis=-1)
        # image = np.delete(image, 7, axis=-1)
        # image = np.delete(image, 6, axis=-1)
        # image = np.delete(image, 5, axis=-1)
        # image = np.delete(image, 4, axis=-1)
